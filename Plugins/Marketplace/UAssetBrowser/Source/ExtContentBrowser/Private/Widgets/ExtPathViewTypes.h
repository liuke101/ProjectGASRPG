...
struct FTreeItem : public TSharedFromThis<FTreeItem>
{
	...
	FString FolderPath;
	/** The root folder path of the tree item including the name */
	FString RootFolderPath;
	/** If true, this folder is in the process of being named */
	bool bNamingFolder;
	/** If true, this folder is in the process of loading */
	bool bLoading;
	...
	/** Constructor */
	FTreeItem(FText InDisplayName, FString InFolderName, FString InFolderPath, FString InRootFolderPath, TSharedPtr<FTreeItem> InParent, bool InNamingFolder = false)
		: DisplayName(MoveTemp(InDisplayName))
		, FolderName(MoveTemp(InFolderName))
		, FolderPath(MoveTemp(InFolderPath))
		, RootFolderPath(MoveTemp(InRootFolderPath))
